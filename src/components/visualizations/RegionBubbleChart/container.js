...
import { RegionBubbleChart } from "./index";
import MockData from "../../../assets/data/Alabama.mockParticipation.geo.json";
 ...
 *
 *
 * @class RegionBubbleChartContainer
 * @extends {React.Component}
 */
class RegionBubbleChartContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      polygonData: [],
  ...
  componentDidMount() {
    ...
    //removing the demo option because it won't register on the maximize widget view

    if (this.props.options.demo) {
      this.setState({
        mapLayers: [MockData],
        unfilteredMapLayers: [MockData]
      });
    }
  }
  ...
  componentWillReceiveProps(nextprops, props) {
    if (nextprops.graphql && nextprops.graphql.result && nextprops.graphql.result.hieProviderLocations) {
      nextprops.graphql.result.hieProviderLocations.forEach(city => {
        MockData.features.forEach(mockCity => {
          if (mockCity.properties.city === this.capitalizeFirstLetters(city.providerCity)) {
            mockCity.properties.clinicalProviders = city.providerCount;
          } else if (!mockCity.properties.clinicalProviders) {
            mockCity.properties.clinicalProviders = 0;
          } else if (mockCity.properties.city === city.ehrCity) {
            mockCity.properties.ehrProviders = city.ehrCount;
          } else if (!mockCity.properties.ehrProviders) {
            mockCity.properties.ehrProviders = 0;
          }
        });
      });
      this.setState({
    ...
    // if (nextprops.graphql.result.ehrProvidersAggregate) {
    //   nextprops.graphql.result.ehrProvidersAggregate.forEach(city => {
    //     MockData.features.forEach(mockCity => {
    //       if (mockCity.properties.city === city.city) {
    //         mockCity.properties.ehrProviders = city.count;
    //       } else if (!mockCity.properties.ehrProviders) {
    //         mockCity.properties.ehrProviders = 0;
    //       }
    //     })
  ...
  filterCityData(filterPolygon = false, sort = "population", limit = 10) {
    ...
    var features = this.state.regionCityData.features ? this.state.regionCityData.features.slice() : [];
    var sortByPopulation = function(cityA, cityB) {
      ...
      var popA = cityA.properties.population || cityA.properties.pop_2010;
      var popB = cityB.properties.population || cityB.properties.pop_2010;
    ...
    switch (sort) {
      case "population":
      default:
        features = features.sort(sortByPopulation);
    }
    // console.log('filtering with polygon', filterPolygon);
    ...
    //custom filtered top level cities for Alabama
    var customCities = [
      "Florence",
      "Huntsville",
      "Birmingham",
    ...
    if (this.state.atParentLevel) {
      cityLabels.features = _.filter(filteredCities, city => {
        return _.includes(customCities, city.properties.name);
      });
    } else {
      cityLabels.features = filteredCities.slice(0, 20);
    }
  ...
  changeRegion(regionType, region, parentRegion) {
    ...
    if (parentRegion !== undefined && parentRegion !== "") {
      ...
      PubSub.publish("GET FILTERED DATA", { region: { atParentLevel: false, regionName: region.properties.name } });
      ...
      PubSub.publish("GET FILTERED DATA", { region: { atParentLevel: true, regionName: region } });
...
RegionBubbleChartContainer.defaultProps = {
  style: {},
  region: "",
  regionType: "counties"
};
...
// Props validation
RegionBubbleChartContainer.propTypes = {
  style: PropTypes.object,
  region: PropTypes.string,
  regionType: PropTypes.string
};

export default withConfig(graphql(RegionBubbleChartContainer));
