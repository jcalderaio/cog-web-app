...
export class RegisteredDocumentsDetailFilters extends PureComponent {  
  constructor(props) {
    super(props);
      ...
      <div className="panel-default filters-grid-element">
        <Spinner until={!this.props.data.loading && this.props.filters}>
          ...
          <Collapsible header="Organization" isOpen>
            ...
            <div id="filter_organization">
              <Select
                simpleValue
                value={this.props.filters.organization ? this.props.filters.organization : ""}
                onChange={this.handleChangeOrganization}
                options={organizations.map(e => {
                  return { value: e.directDomain, label: e.name };
                })}
...
const graphqlTransform = ({ ownProps, data }) => {
  ...
  const organizations = (data.organization && data.organization.list) || [];
